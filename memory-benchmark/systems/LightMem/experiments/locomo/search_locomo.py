...
def process_sample(
    ...
    for qa_idx, qa in enumerate(sample['qa']):
        ...
        try:
            ...
            if disable_thinking:
                # Qwen3 思考模型：通过 chat template 强制关闭思考（vLLM 支持）
...
def main():
    ...
    parser.add_argument('--disable-thinking', action='store_true',
                       help="Force-disable thinking via chat_template_kwargs (Qwen3 on vLLM). "
                            "Judge calls are NOT affected (judge is external to the memory system).")
