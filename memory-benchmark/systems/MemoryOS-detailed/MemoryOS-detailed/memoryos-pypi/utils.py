...
_embedding_api_client = None  # 用于存储 API embedding 的 OpenAI 客户端

def set_embedding_api_client(client):
    """
    设置用于 API embedding 的 OpenAI 客户端。
    
    :param client: OpenAIClient 实例，用于调用 embedding API
    """
    global _embedding_api_client
    _embedding_api_client = client
    print(f"Embedding API client set. Base URL: {client.base_url}")
...
def _get_embedding_via_api(text, model_name, use_cache=True):
    ...
    :param text: 输入文本
    :param model_name: API 上的模型名称，如 'Qwen/Qwen3-Embedding-8B'
...
def get_embedding(text, model_name="all-MiniLM-L6-v2", use_cache=True, use_api=False, **kwargs):
    ...
    :param text: 输入文本。
    :param model_name: 模型名称（本地或API上的模型名称）。
    :param use_cache: 是否使用内存缓存。
    :param use_api: 如果为 True，则通过 OpenAI 兼容 API 调用 embedding 服务。
