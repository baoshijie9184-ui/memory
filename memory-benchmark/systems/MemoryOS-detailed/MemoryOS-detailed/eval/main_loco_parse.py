...
# 导入配置
from config import API_KEY, BASE_URL, LLM_MODEL, EMBEDDING_MODEL, USE_EMBEDDING_API, H_THRESHOLD
...
def main():
    ...
    if os.path.exists(output_file):
        with open(output_file, "r", encoding="utf-8") as f:
            results = json.load(f)
        print(f"续跑：已加载 {len(results)} 条已有结果")
    else:
        results = []
    done_q = {(r["sample_id"], r["question"]) for r in results}
    ...
    for idx, sample in enumerate(dataset):
        ...
        qa_pairs = sample["qa"]
        pending = [qa for qa in qa_pairs if (sample_id, qa["question"]) not in done_q]
        print(
            f"正在处理样本 {idx + 1}/{total_samples}: {sample_id} "
            f"（已完成 {len(qa_pairs) - len(pending)}/{len(qa_pairs)}）"
        )
        if not pending:
            print(f"{sample_id} 全部问答已完成，跳过")
            continue
        ...
        short_mem = ShortTermMemory(max_capacity=7, file_path=f"mem_tmp_loco_final/{sample_id}_short_term.json")
        mid_mem = MidTermMemory(max_capacity=200, file_path=f"mem_tmp_loco_final/{sample_id}_mid_term.json")
        ...
        skip_ingest = bool(mid_mem.sessions)
        if skip_ingest:
            print(f"{sample_id} 记忆已存在（{len(mid_mem.sessions)} 个 session），跳过灌库")
        else:
            print(f"{sample_id} 开始灌库，共 {len(processed_dialogs)} 轮对话")
        ...
        for qa_idx, qa in enumerate(qa_pairs):
            ...
            if (sample_id, question) in done_q:
                continue
