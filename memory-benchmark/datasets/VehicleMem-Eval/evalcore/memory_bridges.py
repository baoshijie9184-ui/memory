...
class Mem0Bridge:
    ...
    - delete_all: 等价 delete_memoryos_user
    """
...
def build_mem0(model_cfg, llm_client, storage_root=None):
    """storage_root: qdrant 本地目录，默认放在 VehicleMem-Eval/results/mem_data/<model>_mem0"""
    if storage_root is None:
        root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        storage_root = os.path.join(root, "results", "mem_data", f"mem0_{model_cfg.name}")
